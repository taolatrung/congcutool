        ...
        <div className="space-y-12 pb-12">
            ...
            <section className="text-center space-y-6 pt-12 pb-6 px-4 bg-gradient-to-b from-white to-slate-50 border-b border-slate-200/60">
                <div className="inline-flex items-center px-3 py-1 rounded-full bg-brand-50 text-brand-700 text-xs font-medium mb-4 border border-brand-100">
                    ...
                    Hơn 50+ công cụ miễn phí
                </div>
                <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight">
                    Mọi công cụ bạn cần <span className="block text-transparent bg-clip-text bg-gradient-to-r from-brand-600 to-indigo-600 mt-2">ở cùng một nơi</span>
                </h1>
                <p className="text-lg text-slate-600 max-w-2xl mx-auto">
                    Xử lý PDF, hình ảnh, video, và AI tools cực nhanh. Không cần cài đặt. Không giới hạn.
                </p>
                ...
                <div className="max-w-xl mx-auto relative group mt-8">
                    ...
                    <input
                        ...
                        placeholder="Tìm công cụ (Ví dụ: PDF to Word, Nén ảnh...)"
                        value={searchTerm}
            ...
            <section className="container mx-auto px-4 min-h-[400px]">
                <div className="flex items-center gap-2 mb-6">
                    {activeCategory === 'ALL' && !searchTerm ? (
                        <>
                            ...
                            <h2 className="text-xl font-bold text-slate-800">Công cụ phổ biến nhất</h2>
                        </>
                    ) : (
                        <>
                            <Filter className="w-5 h-5 text-brand-600" />
                            <h2 className="text-xl font-bold text-slate-800">
                                {searchTerm ? `Kết quả cho "${searchTerm}"` : activeCategory}
                            </h2>
                        </>
                    )}
                </div>
                ...
                {filteredTools.length === 0 && (
                     <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                        <p className="text-slate-500 font-medium">Không tìm thấy công cụ phù hợp.</p>
                        <button onClick={() => {setSearchTerm(''); setActiveCategory('ALL');}} className="mt-2 text-brand-600 hover:underline text-sm">Xem tất cả công cụ</button>
                     </div>
                )}
            </section>
                        ...
                        return (
                            <section key={cat} className="container mx-auto px-4">
                                <div className="flex items-center gap-2 mb-6 border-b border-slate-200 pb-2">
                                    <h2 className="text-lg font-bold text-slate-700">{cat}</h2>
